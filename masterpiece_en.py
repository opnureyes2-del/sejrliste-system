...
class SejrConverter:
 ...
 "pdf": "[BOOK] PDF",
 "text": "[TEXT] Tekst",
 "command": "[CODE] Kommando",
 }

 CONTROL_MODES = {
 "manual": "[STOP] Manuel - Du styrer ALT",
 "kv1nt": "[AI] Kv1nt - AI-assisteret med forslag",
 "admiral": "[ADMIRAL] Admiral - Fuld automatisk",
 }
...
## [TARGET] DIN ENESTE OPGAVE RIGHT NOW
...
class ChatMessage(Gtk.Box):
 ...
 emoji = avatar_emojis.get(sender.lower(), "[CHAT]")
 ...
 avatar_label.set_markup('<span size="large">[USER]</span>')
 avatar_box.append(avatar_label)
 self.append(avatar_box)
...
class AchievementBadge(Gtk.Box):
 ...
 # Icon
 icon_label = Gtk.Label(label=ach.get("icon", "[UNKNOWN]"))
...
class AchievementPanel(Gtk.Box):
 ...
 trophy = Gtk.Label(label="[VICTORY]")
 trophy.set_markup('<span size="large">[VICTORY]</span>')
...
class LiveActivityMonitor(Gtk.Box):
 ...
 # WHAT
 self.hvad_label = Gtk.Label(label="[LIST] WHAT: Waiter...")
 ...
 # WHEN
 self.hvornaar_label = Gtk.Label(label="[TIME] WHEN: Nu")
 ...
 self.hvad_label.set_text(f"[LIST] {action[:30]}")
 self.hvor_label.set_text(f" {sejr_name[:20]}")
 self.hvornaar_label.set_text(f"[TIME] {now}")

 def _add_activity(self, source: str, message: str, icon: str = "[PIN]"):
 ...
 break
 self.activities = []
 self._add_activity("system", "Activityslog ryddet", "[CLEAN]")

 def log_event(self, source: str, message: str, icon: str = "[PIN]"):
...
class SejrFilManager(Gtk.Box):
 ...
 # Drop hint
 drop_hint = Gtk.Label(label="[FOLDER] Drag files here or use the Import button")
...
class PrioritetsOverblik(Gtk.Box):
 ...
 if priorities["akut"]:
 self._add_section("[ERROR] AKUT", priorities["akut"], "error")
 if priorities["opmaerksomhed"]:
 self._add_section("[WARN] OPMÆRKSOMHED", priorities["opmaerksomhed"], "warning")
 if priorities["naeste"]:
 self._add_section("[ACTIVE] NÆSTE SKRIDT", priorities["naeste"], "accent")
...
class MasterpieceWindow(Adw.ApplicationWindow):
 ...
 self.filter_active = Gtk.ToggleButton(label="[ACTIVE] Active")
 ...
 # Sort dropdown
 sort_model = Gtk.StringList.new(["[DATE] Date", "[DATA] Score", " Name"])
 ...
 (f"{stats['completed_checkboxes']}", "[OK] Complete", "intuition"),
 ]
 ...
 # Tip
 tip_label = Gtk.Label(label="[IDEA] Tip: Brug Ctrl+N for hurtig ny victory")
 ...
 w5_group.set_title("[LIST] 5W KONTROL")
 ...
 hvad_row.set_title("[UNKNOWN] WHAT")
 ...
 hvorfor_row.set_title("[TARGET] WHY")
 ...
 hvordan_row.set_title("[CONFIG] HOW")
 ...
 hvornaar_row.set_title("[TIME] WHEN")
 ...
 progress_group.set_title("[DATA] Fremgang")
 ...
 # Open folder button
 folder_btn = Gtk.Button(label="[DIR] Open Folder")
 ...
 if sejr_file.exists():
 edit_btn = Gtk.Button(label="[TEXT] Rediger Victory")
 ...
 term_btn = Gtk.Button(label="[CODE] Terminal")
 ...
 chat_group.set_title("[CHAT] Activity Stream")
 ...
 self.activity_monitor.log_event("system", f"Indlæste {len(self.sejrs)} victorys", "[SYNC]")
 ...
 active_header = Gtk.Label(label="[ACTIVE] AKTIVE")
 active_header.add_css_class("caption")
 active_header.add_css_class("dim-label")
 active_header.set_halign(Gtk.Align.START)
 active_header.set_margin_start(12)
 ...
 header_text = "[ACTIVE] AKTIVE" if self.current_filter == "active" else "[OK] ARKIVEREDE"
 ...
 dialog.set_title("[SYNC] Universal Victory Converter")
 ...
 input_group.set_title("[INBOX] INPUT TYPE")
 ...
 # === 5W KONTROL ===
 w5_group = Adw.PreferencesGroup()
 w5_group.set_title("[TARGET] 5W KONTROL")
 ...
 send_notification("[DIR] File Browser", "Kopiér stien til det du vil konvertere")
 ...
 "success_msg": "[AI] Nye patterns lært og gemt!",
 "title": "[AI] Patterns",
 ...
 "success_msg": "[VICTORY] SEJR ARKIVERET! Du er fantastisk!",
 "title": "[VICTORY] Archived",
 ...
 send_notification("[FAIL] Error", f"Script fejlede: {e}")
 print(f"Error: {e}")
 ...
 dialog = Adw.MessageDialog(
 transient_for=self,
 heading="[VICTORY] SEJR ARKIVERET!",
...
• Grand Admirals: {stats['grand_admirals']} [MEDAL]
...
Du er på vej mod Admiral niveau!"""
 ...
 Gio.FileMonitorEvent.DELETED: ("[DELETE]", "Deletetet"),
 Gio.FileMonitorEvent.CHANGED: ("[TEXT]", "Ændret"),
 Gio.FileMonitorEvent.MOVED_IN: ("[INBOX]", "Movetet ind"),
 Gio.FileMonitorEvent.MOVED_OUT: ("[EXPORT]", "Movetet ud"),
 }
 icon, action = event_icons.get(event_type, ("[PIN]", "Handling"))
 ...
 # Log til activitysmonitor
 if hasattr(self, 'activity_monitor'):
 self.activity_monitor.log_event("drag-drop", f"Fil droppet: {Path(path).name[:40]}", "[FOLDER]")
