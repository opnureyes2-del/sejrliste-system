...
- ❌ Gøre samme opgave forkert
- ❌ Springe punkter over
- ❌ Overse hvor vi er
- ❌ Færdiggøre noget der ikke er gjort rigtigt
...
class EnforcementState:
    ...
    PENDING_VERIFY = "⚠️ PENDING"   # Done but needs verification
    VERIFIED = "✅ VERIFIED"        # Verified and complete
    BLOCKED = "🚫 BLOCKED"          # Blocked by failed verification
    SKIPPED_TEMP = "⏸️ TEMP SKIP"   # Temporarily skipped (must return)
...
class EnforcementEngine:
    ...
    def verify_checkpoint(self, checkpoint_id: str, proof: Optional[str] = None) -> Tuple[bool, str]:
        ...
        if cp.verification_type == "command" and cp.verification_command:
            ...
            if not proof:
                ...
                self._save_state()
                return False, f"⚠️ AFVENTER MANUEL BEKRÆFTELSE\n\nDu SKAL levere bevis for:\n{cp.name}\n\nHvad er dit bevis?"
        ...
        return True, f"✅ VERIFICERET: {cp.name}\n\nBevis: {cp.verification_proof}"
    ...
    def skip_checkpoint(self, checkpoint_id: str, reason: str) -> Tuple[bool, str]:
        ...
        return True, f"⏸️ MIDLERTIDIGT SPRUNGET OVER: {cp.name}\n\n⚠️ DU SKAL VENDE TILBAGE TIL DENNE OPGAVE!\n\nGrund: {reason}"
    ...
    def get_current_position(self) -> dict:
        ...
        return {
            ...
            'current_checkpoint': {
                ...
                'state': current_cp.state if current_cp else "✅ COMPLETE"
            },
    ...
    def get_visual_timeline(self) -> str:
        ...
        if pos['skipped_must_return'] > 0:
            lines.append(f"⚠️ SKAL TILBAGE TIL {pos['skipped_must_return']} OPGAVER!")
        if pos['can_complete']:
            lines.append("✅ KAN ARKIVERES!")
    ...
    def can_archive(self) -> Tuple[bool, str]:
        ...
        return True, "✅ ALLE CHECKPOINTS VERIFICERET - KAN ARKIVERES!"
