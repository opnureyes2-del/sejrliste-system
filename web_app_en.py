...
/* 🎛️ STREAMLIT ELEMENTS - Premium Override */
...
if st.session_state.view == 'library' or st.session_state.selected_sejr is None:
    ...
    if ENFORCEMENT_AVAILABLE:
        ...
        ">
            ...
            {'<div style="color: var(--accent-red); margin-top: 0.75rem; font-weight: bold;">⚠️ ' + pos["blocking_reason"] + '</div>' if pos['blocking_reason'] else ''}
        </div>
        """, unsafe_allow_html=True)
    ...
    with col1:
        if st.button("✅ Verify (v)", use_container_width=True, type="primary"):
            ...
            for file in sorted(sejr_path.iterdir()):
                if file.is_file():
                    ...
                    with st.expander(f"{icon} {file.name}", expanded=False):
                        if st.button(f"👁️ View", key=f"view_{file.name}"):
    ...
    st.markdown("### 🗓️ Komplet Tidslinje")
