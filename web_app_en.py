...
with st.sidebar:
 ...
 with col2:
 if st.button("[VICTORY] Archive", use_container_width=True):
 st.session_state.view = 'library'
 ...
 if archived_sejrs:
 st.markdown(f"**[VICTORY] Archived ({len(archived_sejrs)})**")
...
if st.session_state.view == 'library' or st.session_state.selected_sejr is None:
 ...
 # ═══════════════════════════════════════════════════════════════════════════
 # [LOCK] ENFORCEMENT PANEL - IMPOSSIBLE TO FAIL
 # ═══════════════════════════════════════════════════════════════════════════
 ...
 st.error(f"""
 [BLOCKED] **BLOCKED!** You have {pos['skipped_must_return']} task(s) you MUST return to!
 ...
 proof = st.text_input(
 f"[TEXT] Proof for: {current_cp.name[:40]}",
 ...
 st.error("[BLOCKED] YOU MUST PROVIDE PROOF!")
 else:
 ...
 if not can_archive:
 st.error(f"[BLOCKED] **ARCHIVING BLOCKED!**\n\n{block_reason}")
 st.stop()
 ...
 with st.expander("[DATA] See all 20 steps", expanded=False):
 ...
 current_marker = " ← YOU ARE HERE" if step.status.value == "[ACTIVE]" else ""
 st.markdown(f"""
 <div style="{style}">
 ...
 # Final outcome
 st.success(f"[TARGET] Expected result: {timeline.final_outcome}")
...
with col_realtime2:
 st.markdown("#### [TARGET] Upcoming Victories")
...
with col_realtime3:
 st.markdown("#### [CODE] Live Code Viewer")
...
with col_refresh1:
 ...
 [IDEA] Tip: Use the button to refresh live data or enable auto-refresh
 </div>
 """, unsafe_allow_html=True)
with col_refresh2:
