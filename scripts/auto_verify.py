...
REGEL: Kan IKKE arkiveres før alle 3 passes er gennemført og verificeret.
"""
...
def print_pass_status(results: dict):
    ...
    print("\n" + "=" * 60)
    print("📊 3-PASS KONKURRENCE STATUS")
    print("=" * 60)
    ...
    # Pass 1
    p1 = results["pass_1"]
    status = "✅" if p1["complete"] else "🔵" if current == 1 else "⏳"
    ...
    # Pass 2
    p2 = results["pass_2"]
    status = "✅" if p2["complete"] else "🔵" if current == 2 else "⏳"
    ...
    if p2["improved_from_pass1"] is not None:
        imp_status = "✅" if p2["improved_from_pass1"] else "❌ SKAL FORBEDRES!"
        print(f"   Forbedring fra Pass 1: {imp_status}")

    # Pass 3
    p3 = results["pass_3"]
    status = "✅" if p3["complete"] else "🔵" if current == 3 else "⏳"
    ...
    if p3["improved_from_pass2"] is not None:
        imp_status = "✅" if p3["improved_from_pass2"] else "❌ SKAL FORBEDRES!"
        print(f"   Forbedring fra Pass 2: {imp_status}")

    # Final verification
    fv = results["final_verification"]
    status = "✅" if fv["complete"] else "⏳"
    print(f"\n{status} FINAL VERIFICATION")
    print(f"   Tests: {fv['done']}/{fv['total']} (minimum 5 krævet)")

    # Total
    print("\n" + "-" * 60)
    print(f"📈 TOTAL SCORE: {results['total_score']}/30 (minimum 24 krævet)")

    # Archive status
    print("\n" + "-" * 60)
    if results["can_archive"]:
        print("✅ KAN ARKIVERES - Alle 3 passes gennemført!")
    else:
        print("🔒 ARKIVERING BLOKERET - Mangler:")
        if not results["pass_1"]["complete"]:
            print("   • Pass 1 ikke færdig")
        if not results["pass_2"]["complete"]:
            print("   • Pass 2 ikke færdig")
        elif results["pass_2"]["improved_from_pass1"] == False:
            print("   • Pass 2 score SKAL være højere end Pass 1")
        if not results["pass_3"]["complete"]:
            print("   • Pass 3 ikke færdig")
        elif results["pass_3"]["improved_from_pass2"] == False:
            print("   • Pass 3 score SKAL være højere end Pass 2")
...
def verify_all(system_path: Path):
    ...
    if ready_to_archive:
        print(f"\n✅ READY TO ARCHIVE ({len(ready_to_archive)}):")
