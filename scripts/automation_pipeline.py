...
def run_ollama_review(filepath):
 ...
 print(f"{GREEN}[LLAMA] Code Review:{RESET}")
 print(review[:1000])
...
def generate_report(filepath, results, duration):
 ...
 print(f"\n{BOLD}{'='*60}{RESET}")
 print(f"{BOLD}[DATA] PIPELINE RAPPORT: {filename}{RESET}")
 print(f"{'='*60}")
 print(f"[DATE] Dato: {datetime.now().strftime('%Y-%m-%d %H:%M')}")
 print(f"⏱ Varighed: {duration:.1f}s")
 print(f"[LIST] Steps: {passed}/{total} passed")
 ...
 if score >= 8:
 print(f"{GREEN}[VICTORY] KVALITET: GOD — klar til commit{RESET}")
 elif score >= 5:
 print(f"{YELLOW}[TOOL] KVALITET: OK — fix issues før commit{RESET}")
 else:
 print(f"{RED}[ALERT] KVALITET: LAV — kræver arbejde{RESET}")
