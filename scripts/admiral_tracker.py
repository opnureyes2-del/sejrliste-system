...
def log_event(sejr_path: Path, event_type: str, note: str = ""):
    ...
    if not score_file.exists():
        print(f"❌ ADMIRAL_SCORE.yaml not found in {sejr_path}")
        return
    ...
    # Print result
    symbol = "✅" if section == "positive" else "❌"
...
def show_score(sejr_path: Path):
    ...
    if not score_file.exists():
        print(f"❌ No ADMIRAL_SCORE.yaml in {sejr_path}")
        return
    ...
    print("\n" + "=" * 60)
    print(f"🎖️  ADMIRAL SCORE: {sejr_path.name}")
    print("=" * 60)
...
def finalize_sejr(sejr_path: Path):
    ...
    print(f"\n🎖️ FINAL SCORE: {data['totals']['total_score']}")
    print(f"🎖️ FINAL RANK: {data['totals']['rank']}")
...
if __name__ == "__main__":
    ...
    if args.leaderboard:
        ...
        if not sejr_path.exists():
            print(f"❌ Sejr not found: {args.sejr}")
        elif args.event:
