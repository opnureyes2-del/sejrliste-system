...
def write_yaml_simple(filepath: Path, data: dict):
 ...
 lines.append(f"# Updated: {datetime.now(timezone.utc).astimezone().isoformat()}")
 lines.append("")
...
def show_score(sejr_path: Path):
 ...
 print("\n" + "=" * 60)
 print(f"[ADMIRAL] ADMIRAL SCORE: {sejr_path.name}")
 print("=" * 60)

 print(f"\n[DATA] MODEL: {data.get('meta', {}).get('model_name', 'Unknown')}")
 ...
 print(f"\n[VICTORY] ACHIEVEMENTS: +{totals['achievement_bonus']}")

 print("\n" + "-" * 40)
 print(f" TOTAL SCORE: {totals['total_score']}")
 print(f"[ADMIRAL] RANK: {totals['rank']}")
 print("=" * 60 + "\n")
...
def show_leaderboard(system_path: Path):
 ...
 print("\n" + "=" * 70)
 print("[VICTORY] ADMIRAL LEADERBOARD")
 print("=" * 70)
...
def finalize_sejr(sejr_path: Path):
 ...
 print("[VICTORY] Achievement unlocked: FLAWLESS SEJR (+50)")
 ...
 print(f"\n[ADMIRAL] FINAL SCORE: {data['totals']['total_score']}")
 print(f"[ADMIRAL] FINAL RANK: {data['totals']['rank']}")
