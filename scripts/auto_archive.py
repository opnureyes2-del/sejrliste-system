...
def write_yaml_simple(filepath: Path, data: dict):
 ...
 lines = [f"# Archive metadata - {datetime.now(timezone.utc).astimezone().isoformat()}"]
...
def extract_semantic_conclusion(sejr_path: Path) -> str:
 ...
 result_start = content.find("# [DATA] 3-PASS KONKURRENCE RESULTAT")
 if result_start == -1:
 result_start = content.find("3-PASS KONKURRENCE RESULTAT")

 # Extract SEMANTISK KONKLUSION
 conclusion_start = content.find("# [VICTORY] SEMANTISK KONKLUSION")
 ...
 if conclusion_start != -1:
 conclusion_end = content.find("\n## [LOCK] ARCHIVE LOCK", conclusion_start)
 if conclusion_end == -1:
...
def get_rank_from_score(total_score: int) -> tuple[str, str]:
 ...
 if total_score >= 27:
 return "GRAND ADMIRAL", "[MEDAL]"
 elif total_score >= 24:
 return "ADMIRAL", "[ADMIRAL]"
 elif total_score >= 21:
 return "KAPTAJN", ""
 elif total_score >= 18:
 return "LØJTNANT", "[DATA]"
 else:
 return "KADET", ""
...
def count_checkboxes_per_pass(content: str) -> dict:
 ...
 p1_match = re.search(r'# PASS 1.*?(?=# [SCAN] PASS 1|# PASS 2|\Z)', content, re.DOTALL | re.IGNORECASE)
 if p1_match:
 p1_section = p1_match.group(0)
 result['pass_1'] = len(re.findall(r'\[x\]', p1_section, re.IGNORECASE))

 # Find PASS 2 section
 p2_match = re.search(r'# PASS 2.*?(?=# [SCAN] PASS 2|# PASS 3|\Z)', content, re.DOTALL | re.IGNORECASE)
 if p2_match:
 p2_section = p2_match.group(0)
 result['pass_2'] = len(re.findall(r'\[x\]', p2_section, re.IGNORECASE))

 # Find PASS 3 section
 p3_match = re.search(r'# PASS 3.*?(?=# [OK] FINAL|# [DATA] 3-PASS|\Z)', content, re.DOTALL | re.IGNORECASE)
 if p3_match:
...
def extract_pass_approaches(content: str) -> dict:
 ...
 p1_complete = re.search(r'## [LIST] PASS 1 COMPLETION CHECKLIST.*?### PASS 1 SCORE: (\d+)/10.*?Tid brugt.*?: ([^\n]+)', content, re.DOTALL)
 if p1_complete:
 ...
 p1_review = re.search(r'# [SCAN] PASS 1 → PASS 2 REVIEW.*?## Hvad Virker\?.*?\n(1\.[^\n]+(?:\n2\.[^\n]+)?(?:\n3\.[^\n]+)?)', content, re.DOTALL)
 if p1_review:
...
def extract_learnings_and_patterns(sejr_path: Path) -> dict:
 ...
 conclusion_start = content.find("SEMANTISK KONKLUSION")
 if conclusion_start == -1:
 conclusion_start = content.find("[VICTORY] SEMANTISK")
...
def generate_sejr_diplom(sejr_path: Path, archive_path: Path, status: dict) -> Path:
 ...
 diplom_content = f"""# [VICTORY] SEJR DIPLOM
...
- **Archive timestamp:** {datetime.now(timezone.utc).astimezone().isoformat()}
- **3-Pass verified:** [OK]
"""
...
def archive_sejr(sejr_name: str, system_path: Path, force: bool = False):
 ...
 print(f"\n[LOCK] ARKIVERING BLOKERET for: {sejr_name}")
 print(f"\n Årsag:{check['reason']}")
 ...
 print(f"[DIR] Archive location: {archive_path}")
 ...
 print(f"[VICTORY] Generated diplom: {diplom_file}")
 ...
 print(f"\n[DELETE] Removing from 10_ACTIVE: {sejr_path}")
...
def update_archive_index(system_path: Path):
 ...
 content = f"""# [VICTORY] SEJRLISTE ARKIV INDEX
...
"""
 ...
 diplom_link = f"[[VICTORY]]({sejr['name']}/SEJR_DIPLOM.md)" if sejr['has_diplom'] else "[FAIL]"
...
├── SEJR_DIPLOM.md # [VICTORY] Bevis og showcase
...
"""
 ...
 print(f"[LIST] Updated: {index_file}")
 return index_file
...
def list_completed_sejr(system_path: Path):
 ...
 if not_ready:
 print(f"\n[LOCK] NOT READY ({len(not_ready)}):\n")
 for sejr in not_ready:
 print(f" [PENDING] {sejr['name']}")
...
if __name__ == "__main__":
 ...
 rank_name, rank_emoji = get_rank_from_score(status.get('total_score', 0))
 print(f"[VICTORY] Generated: {folder.name} ({rank_emoji} {rank_name})")
 generated += 1
