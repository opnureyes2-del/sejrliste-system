...
def update_claude_md(sejr_path: Path):
 ...
 pass_status = ["[PENDING] Pending", "[PENDING] Pending", "[PENDING] Pending"]
 if current_pass == 1:
 pass_status[0] = "[ACTIVE] In Progress"
 elif current_pass == 2:
 pass_status[0] = "[OK] Complete"
 pass_status[1] = "[ACTIVE] In Progress"
 elif current_pass == 3:
 pass_status[0] = "[OK] Complete"
 pass_status[1] = "[OK] Complete"
 pass_status[2] = "[ACTIVE] In Progress"
 ...
 # Print summary
 print(f"\n{'─' * 40}")
 print(f"[LOCK] CLAUDE FOKUS OPDATERET")
 print(f"{'─' * 40}")
...
def update_all(system_path: Path):
 ...
 print(f"[SYNC] Updating CLAUDE.md for {len(sejr_folders)} sejr...\n")
