...
def update_claude_md(sejr_path: Path):
    ...
    if not status:
        print(f"⚠️  No STATUS.yaml found in {sejr_path}")
        return False
    ...
    if template_path.exists():
        ...
        print(f"⚠️  No CLAUDE.md template found")
        return False
    ...
    if current_pass == 1:
        ...
        pass_status[0] = "✅ Complete"
        pass_status[1] = "🔵 In Progress"
    elif current_pass == 3:
        pass_status[0] = "✅ Complete"
        pass_status[1] = "✅ Complete"
    ...
    if status.get("pass_3_complete"):
        pass_status[2] = "✅ Complete"

    # Status text
    if status.get("can_archive"):
        status_text = "✅ KLAR TIL ARKIVERING"
    else:
...
def update_all(system_path: Path):
    ...
    if not active_dir.exists():
        print("❌ No 10_ACTIVE directory")
        return

    sejr_folders = [f for f in active_dir.iterdir() if f.is_dir()]

    if not sejr_folders:
        print("ℹ️  No active sejr found")
        return
...
if __name__ == "__main__":
    ...
    if args.all:
        ...
        if sejr_path.exists():
            ...
            print(f"❌ Sejr not found: {sejr_path}")
    else:
        update_all(system_path)
